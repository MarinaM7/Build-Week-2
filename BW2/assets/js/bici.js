...
function printListino() {
  fetch('http://localhost:3000/bici').then((response) => {
...
function totalCost(product) {
  ...
  if (cartCost != null) {
    ...
    localStorage.setItem('totalCost', product.prezzo);

  }

}

// Display carrello adesso funziona piu o meno, vanno sistemate alcune cose tipo il tasto rimuovi prodotto, funzione usata sull'HTML di bici
function displayCart() {
  ...
  if (cartItems1 && productContainer) {
    ...
    <tr>
        <td colspan="2"><h5 class="text-end me-2">Totale: <span class="text-warning"> &euro;${Math.round((cartCost1*100)/100)}</span></h5></td>
    </tr>
</tfoot>`;
  }
}
...
function instantSearch() {
  document.querySelectorAll('.col-xl-3').forEach(item => item.querySelectorAll('h5')[0].innerText.toLowerCase().indexOf(document.querySelector('#input').value.toLowerCase()) > -1 ?
